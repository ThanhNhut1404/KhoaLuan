...
<style>
    ...
    #participatedTermFilter {
        min-width: 188px;
    }

    #participatedTypeFilter,
    #participatedLevelFilter,
    #participatedStatusFilter {
        min-width: 132px;
    }
    ...
    <div class="joined-card card">
        <div class="joined-panel__header card-header">
            ...
            <div class="joined-toolbar">
                <button class="filter-toggle btn btn-outline-secondary" id="participatedFilterToggle" type="button" title="Bộ lọc" aria-label="Bộ lọc" aria-expanded="false">
                    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <path d="M4 5h16l-6 7v5l-4 2v-7L4 5Z" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
                </button>

                <div class="filter-panel" id="participatedFilterPanel" aria-hidden="true">
                    <div class="filter-grid">
                        <label class="filter-field">
                            <span>Học kỳ</span>
                            <select class="filter-select form-select" id="participatedTermFilter" aria-label="Lọc theo học kỳ">
                        ...
                        <label class="filter-field">
                            ...
                            <select class="filter-select form-select" id="participatedTypeFilter" aria-label="Lọc theo loại hoạt động">
                                <option value="">Tất cả</option>
                        ...
                        <label class="filter-field">
                            ...
                            <select class="filter-select form-select" id="participatedLevelFilter" aria-label="Lọc theo cấp hoạt động">
                                <option value="">Tất cả</option>
                        ...
                        <label class="filter-field">
                            ...
                            <select class="filter-select form-select" id="participatedStatusFilter" aria-label="Lọc theo trạng thái">
                                <option value="">Tất cả</option>
                    ...
                    <div class="filter-actions">
                        <button class="filter-btn filter-reset-btn btn btn-outline-secondary" id="participatedFilterReset" type="button">Đặt lại</button>
                        <button class="filter-btn filter-apply-btn btn btn-primary" id="participatedFilterApply" type="button">Áp dụng</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="table-responsive"><table class="joined-table table table-hover table-bordered align-middle">
            <thead>
                <tr>
                    ...
                    <th class="cell-center col-score">Điểm</th>
            ...
            <tbody>
                <?php if (!empty($activities)): ?>
                    <?php foreach ($activities as $index => $activity): ?>
                        <tr
                            ...
                            data-status="<?= htmlspecialchars($activity['status'] ?? 'Đã tham gia', ENT_QUOTES, 'UTF-8') ?>"
                        >
                            ...
                            <td class="col-level"><span class="badge level rounded-pill"><?= htmlspecialchars($activity['level'] ?? '') ?></span></td>
                            <td class="cell-center col-status"><span class="badge rounded-pill"><?= htmlspecialchars($activity['status'] ?? 'Đã tham gia') ?></span></td>
                            <td class="cell-center col-score"><span class="badge score rounded-pill"><?= htmlspecialchars($activity['score'] ?? '') ?></span></td>
                            <td class="cell-center col-actions">
                                <div class="action-list">
                                    <button class="action-btn view btn" type="button" title="Xem" aria-label="Xem">
                                        <i class="fa-solid fa-eye"></i>
                                    </button>
                                    <button class="action-btn proof btn" type="button" title="Minh chứng" aria-label="Minh chứng">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="9" class="empty-state">Chưa có hoạt động nào đã tham gia.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table></div>

            <div class="pagination-container" id="participatedPaginationContainer">
                <div class="pagination-info" id="participatedPaginationInfo"></div>
                <div class="pagination mb-0" id="participatedPagination"></div>
            </div>
        </div>
    </div>
</div>
...
<script>
    (function() {
        const rows = Array.from(document.querySelectorAll('.joined-table tbody tr[data-participated-row]'));
        const pagination = document.getElementById('participatedPagination');
        const paginationInfo = document.getElementById('participatedPaginationInfo');
        const tbody = document.querySelector('.joined-table tbody');
        const toggle = document.getElementById('participatedFilterToggle');
        const panel = document.getElementById('participatedFilterPanel');
        const applyBtn = document.getElementById('participatedFilterApply');
        const resetBtn = document.getElementById('participatedFilterReset');
        const termFilter = document.getElementById('participatedTermFilter');
        const typeFilter = document.getElementById('participatedTypeFilter');
        const levelFilter = document.getElementById('participatedLevelFilter');
        const statusFilter = document.getElementById('participatedStatusFilter');
        const itemsPerPage = 10;
        let currentPage = 1;
        let emptyRow = document.getElementById('participatedPaginationEmptyRow');

        if (!tbody || !pagination || !paginationInfo) return;

        if (!emptyRow) {
            emptyRow = document.createElement('tr');
            emptyRow.id = 'participatedPaginationEmptyRow';
            emptyRow.style.display = 'none';
            emptyRow.innerHTML = '<td colspan="9" class="empty-state">Chưa có hoạt động nào đã tham gia.</td>';
        ...
        function getFilteredRows() {
            return rows.filter(function(row) {
                return matches(row, 'term', termFilter ? termFilter.value : '') &&
                    matches(row, 'type', typeFilter ? typeFilter.value : '') &&
                    matches(row, 'level', levelFilter ? levelFilter.value : '') &&
                    matches(row, 'status', statusFilter ? statusFilter.value : '');
            });
        }
        ...
        if (toggle && panel && applyBtn && resetBtn) {
