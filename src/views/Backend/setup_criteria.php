        ...
        <div class="panel-body card-body">
                ...
                <div class="d-flex gap-2">
                    <?php if ($selectedTemplateId > 0): ?>
                        ...
                        <form method="POST" action="/KhoaLuan/public/admin.php?page=setup_criteria" class="d-inline">
                            ...
                            <button type="submit" class="btn btn-outline-danger">Khóa bộ mẫu</button>
                        </form>
                        <form method="POST" action="/KhoaLuan/public/admin.php?page=setup_criteria" class="d-inline">
                            <input type="hidden" name="form_type" value="template_master_delete">
                            <input type="hidden" name="template_id" value="<?= htmlspecialchars((string) $selectedTemplateId, ENT_QUOTES, 'UTF-8') ?>">
                            <button type="submit" class="btn btn-outline-danger">Xóa bộ mẫu</button>
                        </form>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <?php if ($selectedTemplateId > 0): ?>
                ...
                <div class="d-flex align-items-center justify-content-between mb-3">
                    <div>
                        <h5 class="mb-0">Bộ tiêu chí: <?= htmlspecialchars($templateName, ENT_QUOTES, 'UTF-8') ?></h5>
                        <div class="text-muted">Tổng điểm: <strong><?= number_format($totalScore, 2) ?> / 100</strong></div>
                    </div>
                </div>
                                ...
                                <div class="card">
                                    <div class="card-header d-flex align-items-center justify-content-between">
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="badge bg-primary text-white" style="min-width:44px;min-height:44px;display:inline-flex;align-items:center;justify-content:center;font-weight:700;font-size:16px;"><?= toRoman($index + 1) ?></div>
                                            <div>
                                                <div style="font-weight:700;color:#0f2a5a"><?= $categoryName ?></div>
                                                <div class="text-muted small">Tối đa: <?= htmlspecialchars((string) ($category['max_points'] ?? $category['DIEM_TOI_DA_MUC'] ?? '0'), ENT_QUOTES, 'UTF-8') ?> điểm • <?= count($items) ?> tiêu chí con</div>
                                            </div>
                                        </div>
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <button class="btn btn-sm btn-outline-success addSubCriteriaButton" type="button" data-category-id="<?= htmlspecialchars((string) $categoryId, ENT_QUOTES, 'UTF-8') ?>" data-category-name="<?= $categoryName ?>" data-bs-toggle="modal" data-bs-target="#addCriteriaModal">+ Thêm tiêu chí con</button>
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <?php if (empty($items)): ?>
                                                    ...
                                                    <tbody>
                                                        <?php foreach ($items as $itemIndex => $item): ?>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <div class="d-flex align-items-center gap-2">
                                                                        <span class="badge bg-<?= $useForActivity === 1 ? 'success' : 'secondary' ?>"><?= $useForActivity === 1 ? 'Có' : 'Không' ?></span>
