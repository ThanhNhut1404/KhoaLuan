...
$value = static fn(string $key): string => htmlspecialchars((string) ($formData[$key] ?? ''), ENT_QUOTES, 'UTF-8');
$selected = static fn(string $key, string $val): string => (string) ($formData[$key] ?? '') === $val ? 'selected' : '';
$error = static fn(string $key): string => '<span class="field-error' . (empty($errors[$key]) ? ' is-empty' : '') . '">' . (!empty($errors[$key]) ? htmlspecialchars($errors[$key], ENT_QUOTES, 'UTF-8') : '&nbsp;') . '</span>';

$selectedDepartmentId = (string) ($formData['department_id'] ?? '');
$selectedMajorId = (string) ($formData['major_id'] ?? '');
$selectedAcademicYearId = (string) ($formData['academic_year_id'] ?? '');
...
<div class="student-form-page">
    <div class="page-panel card">
        <div class="panel-header card-header">
            <h2 class="panel-title">CẬP NHẬT SINH VIÊN</h2>
        </div>

        <div class="panel-body card-body">
            <form method="post" action="/KhoaLuan/public/admin.php?page=edit_student&id=<?= htmlspecialchars((string) ($_GET['id'] ?? ''), ENT_QUOTES, 'UTF-8') ?>">
                <input type="hidden" name="username" value="<?= $value('username') ?>" />

                <div class="form-grid">
                    <div class="form-field">
                        <label class="field-label form-label" for="mssv">MSSV</label>
                        <input id="mssv" class="field-input form-control" type="text" value="<?= $value('mssv') ?>" readonly />
                    ...
                    <div class="form-field">
                        ...
                        <select id="department_id" name="department_id" class="field-input form-select">
                            ...
                            <?php foreach ($listKhoa as $khoa): ?>
                                <option value="<?= htmlspecialchars((string) ($khoa['MA_KHOA'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" <?= $selected('department_id', (string) ($khoa['MA_KHOA'] ?? '')) ?>>
                                    <?= htmlspecialchars((string) ($khoa['TEN_KHOA'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <?= $error('department_id') ?>
                    ...
                    <div class="form-field">
                        ...
                        <select id="academic_year_id" name="academic_year_id" class="field-input form-select" data-selected-value="<?= htmlspecialchars($selectedAcademicYearId, ENT_QUOTES, 'UTF-8') ?>">
                            <option value="">-- Chọn niên khóa --</option>
                            <?php foreach ($listNienKhoa as $nk): ?>
                                <option value="<?= htmlspecialchars((string) ($nk['MA_NIEN_KHOA'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" <?= $selected('academic_year_id', (string) ($nk['MA_NIEN_KHOA'] ?? '')) ?>>
                                    <?= htmlspecialchars((string) ($nk['TEN_NIEN_KHOA'] ?? ''), ENT_QUOTES, 'UTF-8') ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-field">
                        ...
                        <select id="class_id" name="class_id" class="field-input form-select" data-selected-value="<?= htmlspecialchars($selectedClassId, ENT_QUOTES, 'UTF-8') ?>" <?= ($selectedMajorId === '' || $selectedAcademicYearId === '') ? 'disabled' : '' ?>>
                            <option value=""><?= ($selectedMajorId === '' || $selectedAcademicYearId === '') ? 'Vui lòng chọn ngành học và niên khóa trước' : '-- Chọn lớp học --' ?></option>
                        </select>
                        <?= $error('class_id') ?>
                    </div>
                    ...
                    <div class="form-field full-width">
                        ...
                        <select id="status" name="status" class="field-input form-select">
                            <?php foreach ($statusOptions as $option): ?>
                                <option value="<?= htmlspecialchars((string) ($option['value'] ?? ''), ENT_QUOTES, 'UTF-8') ?>" <?= $selected('status', (string) ($option['value'] ?? '')) ?>>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="action-btn primary save-change-btn btn btn-primary">Lưu thay đổi</button>
                </div>
            </form>
        </div>
    </div>
...
<style>
    ...
    .field-input { display: flex; align-items: center; padding: 10px; border: 1px solid #e5e7eb; border-radius: 10px; background: #f9fafb; font-size: 13px; color: #1f2937; font-family: inherit; height: 40px; box-sizing: border-box; width: 100%; transition: border-color 0.2s, box-shadow 0.2s; }
    .field-input:focus { outline: none; border-color: #0f2a5a; box-shadow: 0 0 0 3px rgba(15, 42, 90, 0.08); background: #ffffff; }
    .field-input:disabled,
    .field-input[readonly] { cursor: not-allowed; color: #64748b; background-color: #eef2f7; }
    ...
    .field-error { color: #dc2626; font-size: 12px; min-height: 18px; line-height: 1.2; overflow-wrap: anywhere; }
    ...
    .action-btn { padding: 8px 20px; border-radius: 10px; border: 1px solid #e5e7eb; background: #fff; color: #0f2a5a; font-size: 13px; font-weight: 700; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; justify-content: center; gap: 6px; transition: all 0.2s; white-space: nowrap; }
...
<script>
    (function() {
        ...
        if (!departmentSelect || !majorSelect || !academicYearSelect || !classSelect || !dataNode) {
            return;
        }
        ...
        function populateClasses(selectedValue) {
            ...
            if (majorId === '' || academicYearId === '') {
                replaceOptions(classSelect, 'Vui lòng chọn ngành học và niên khóa trước', [], '');
            ...
            const filteredClasses = classes.filter(function(classItem) {
                return String(classItem.department_id || '') === departmentId
                    ...
                    && String(classItem.year_id || '') === academicYearId;
            });
